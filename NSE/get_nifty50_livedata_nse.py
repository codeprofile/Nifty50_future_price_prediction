...
def get_nifty50_livedata_nse():
    ...
    for index, row in df_n50.iterrows():
        ...
        data = get_history(symbol=stock_code, start=date(current_date.year,current_date.month,current_date.day), end=date(current_date.year,current_date.month,current_date.day))
        current_price.append(data["Last"].values)
    df_n50["current_price"] = pd.DataFrame(current_price)
    return df_n50.to_dict('records')


def get_historical_data_nifty50(startdate:str,enddate:str):
    ...
    for index, row in df_n50.head(1).iterrows():
        ...
        if (regexp.search(stock_code) != None):
            stock_code = stock_code.replace('&', '%26')
        data = get_history(symbol=stock_code, start=date(startdate.year,startdate.month,startdate.day), end=date(enddate.year,enddate.month,enddate.day))
